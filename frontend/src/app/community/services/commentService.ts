...
export async function getCommentsByReviewId(reviewId: number): Promise<Comment[]> {
    try {
        const comments = await api.get<Comment[]>(`/api/comments/review/${reviewId}`);
        return comments || [];
    } catch (error) {
...
// 댓글 생성
export async function createComment(comment: CommentRequestDto): Promise<Comment> {
    try {
...
export async function updateComment(commentId: number, content: string): Promise<Comment> {
    try {
        const updatedComment = await api.put<Comment>(`/api/comments/${commentId}`, { content });
